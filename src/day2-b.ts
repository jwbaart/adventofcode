const main = () => {
    ...
    const structuredData: number[][] = data.map((row) => {
        return row.split("	").map(Number);
    });

    const results = structuredData.map((row, rI) => {
        const rowResult = row.filter((col, cI, cRow) => {
            return cRow.some((element) => {
                return (element % col === 0 || col % element === 0) && col !== element;
            });
        });

        return rowResult[0] > rowResult[1] ? rowResult[0] / rowResult[1] : rowResult[1] / rowResult[0]
    });
